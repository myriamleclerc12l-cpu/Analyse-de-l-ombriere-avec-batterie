...
fichiers_conso = st.sidebar.file_uploader("Courbes de charge (site)", type=["csv", "xlsx", "xls"],
    accept_multiple_files=True,
    help="Un ou plusieurs fichiers, même format que la courbe de charge du siège (2 colonnes : date, "
         "puissance en W). Toutes les courbes sont sommées pour obtenir la consommation totale du site.")
fichier_bornes = st.sidebar.file_uploader("Courbe de charge - Bornes de recharge  :", type=["csv", "xlsx", "xls"])
fichiers_prod = st.sidebar.file_uploader("Courbes de production PV", type=["csv", "xlsx", "xls"],
    accept_multiple_files=True,
    help="Un ou plusieurs fichiers, même format que la courbe de production de l'ombrière (2 colonnes : "
...
# Texte affiché au survol du petit "?" à côté de chaque case
    ...
    with tab5:
        ...
        "Autoconso Totale (kWh)"
                ...
                pdf_buffer = generer_pdf_enolab(df_enolab, capacite_etude, capex_v2,
                ...
                st.download_button(label=" Télécharger le bilan financier en PDF", data=pdf_buffer,
                    file_name=f"bilan_financier_{capacite_etude:.0f}kWh.pdf", mime="application/pdf")

                png_buffer = generer_png_enolab(df_enolab, capacite_etude, capex_v2,
                    f"{tri_v2*100:.1f} %" if tri_v2 is not None else "N/A",
                    f"{lcos_v2*100:.2f} c€/kWh" if not np.isnan(lcos_v2) else "N/A",
                    f"{payback_v2:.1f} ans" if payback_v2 is not None else "N/A",
                    f"{prix_ttc_moyen*100:.2f} c€/kWh")
